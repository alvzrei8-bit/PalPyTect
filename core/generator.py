...
"""
        ...
        junk_code = self._generate_junk(200)
...
def _vmp():
    ...
    __1 = {{'\\x7fELF\\x01': exec}}
    __2 = {{'\\x7fELF\\x02': compile}}
    __3 = {{'\\x7fELF\\x03': hashlib.sha256}}
    __4 = {{'\\ufffd\\x00\\xff': base64.b64decode}}
    ...
    while _ptr:
        ...
        try:
            _rl = __3['\\x7fELF\\x03'](str(_h).encode()).digest()
            ...
            {self._generate_junk(50)}
            __1['\\x7fELF\\x01'](__2['\\x7fELF\\x02'](bytes(_dec), '', 'exec'), _p_ns)
...
"""
    ...
    def _generate_junk(self, count):
        ...
        for _ in range(count):
            ...
            op = random.choice(['+', '-', '^', '*', '//'])
            junk.append(f"__junk_{random.randint(0,1000)} = {k} if {v} {op} {random.randint(1,100)} else {v}")
        return "\\n    ".join(junk)
