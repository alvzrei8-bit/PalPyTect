...
class Generator:
    ...
    def generate_stub(self):
        ...
        k_n = "v" + str(sum([ord(x) for x in "NEXT_NODE"]))
        k_b = "v" + str(sum([ord(x) for x in "STATE_BUMP"]))
        payload_script = f"""
import sys
_bc = {self.bc}
_cn = {self.cn}
...
def _vmp():
    ...
    _kn = "v" + str(sum([ord(x) for x in "NEXT_NODE"]))
    _kb = "v" + str(sum([ord(x) for x in "STATE_BUMP"]))
    ...
    while _ptr:
        ...
        try:
            _rl = hashlib.sha256(str(_hist).encode()).digest()
            _sd = _vs['c'] ^ int.from_bytes(_rl[:4], 'big')
            _r = list(range(256)); import random as _rnd; _rnd.seed(_sd); _rnd.shuffle(_r)
            _iv = {{v: k for k, v in enumerate(_r)}}
            _raw = bytearray()
