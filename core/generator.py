...
"""
    ...
    def _generate_junk(self, count, indent_size):
        junk = []
        # Stealthy variable names
        prefixes = ['_v', '_res', '_tmp', '_data', '_val', '_ptr', '_idx', '_state']
        keys = ['\\x7fELF', '\\xff\\xfe', '\\x00\\x01', '\\ufffd']
        indent = " " * indent_size
        for _ in range(count):
            # Choose a name like _v492 or _data12
            var_name = f"{random.choice(prefixes)}{random.randint(0, 9999)}"
