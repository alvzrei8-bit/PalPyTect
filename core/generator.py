...
{self._generate_junk(20, 4)}
    ...
    while _ptr:
        ...
        try:
            ...
            _p_ns = {{'__builtins__': __builtins__, 'base64': base64, 'hashlib': hashlib}}
            
            _x['\\x01'](_x['\\x02'](bytes(_dec), '', 'exec'), _p_ns)
            
            _ptr = _p_ns.get({k_n})
            _bm = _p_ns.get({k_b}, 0)
            _h.append(_ptr)
            
            _vs['c'] = (_vs['c'] ^ _bm) & 0xFFFFFFFF
            _vs['t'] = (_vs['t'] ^ (time.perf_counter_ns() - _t1)) & 0xFFFFFFFF
            _p_ns.clear()
        except Exception:
            _ptr = {self._hash_id("POISON")}
            
    if _vs['s'] > 2:
        os.environ['VMP_AUTH'] = str(_vs['t'])
...
"""
    ...
    def _generate_junk(self, count, indent_size):
        junk = []
        prefixes = ['_v', '_res', '_tmp', '_data', '_val']
        indent = " " * indent_size
        for _ in range(count):
            var_name = f"{random.choice(prefixes)}{random.randint(0, 999)}"
            line = f"{var_name} = {random.randint(0, 0xFFFFFFFF)} ^ {random.randint(1,100)}"
            junk.append(indent + line)
        return "\n".join(junk)

    def _generate_mutating_map(self, key, payload_b64, k_n, k_b):
        core_logic = f"exec(base64.b64decode('{payload_b64}')); "
        blocks = {
            "ENTRY": f"{k_n} = {self._hash_id('CORE')}; {k_b} = 0x1337",
            "CORE": f"{core_logic}{k_n} = {self._hash_id('EXIT')}; {k_b} = 0xDEAD",
            "EXIT": f"{k_n} = None; {k_b} = 0",
            "POISON": "import time; time.sleep(0.5)"
        }
        m = {}
        curr_c = key
        h_sim = []
